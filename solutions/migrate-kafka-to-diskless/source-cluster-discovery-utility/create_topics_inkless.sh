...
grep "^Topic:" "$INPUT_FILE" | grep "Configs:" | while read -r line; do
    ...
    CMD=(avn service topic-create "$SERVICE_NAME" "$TOPIC" --partitions "$PARTITIONS" --diskless-enable --replication 1)

    if [ -n "$RETENTION" ]; then
        CMD+=(--retention-bytes "$RETENTION" --retention 72) # 24x3=72 hours
    fi
