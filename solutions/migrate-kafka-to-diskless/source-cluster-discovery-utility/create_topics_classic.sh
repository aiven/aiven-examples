...
grep "^Topic:" "$INPUT_FILE" | grep "Configs:" | while read -r line; do
    ...
    # 3. Extract Replication Factor
    # Looks for "ReplicationFactor: " followed by digits
    REPLICATION=$(echo "$line" | sed -n 's/.*ReplicationFactor: \([0-9]*\).*/\1/p')

    # 4. Extract Cleanup Policy
    # Looks for "cleanup.policy=" followed by word characters (like 'compact' or 'delete')
    CLEANUP_POLICY=$(echo "$line" | sed -n 's/.*cleanup.policy=\([a-z]*\).*/\1/p')    

    # 5. Extract the numeric values for the specific properties
    ...
    CMD=(avn service topic-create "$SERVICE_NAME" "$TOPIC" --partitions "$PARTITIONS" --cleanup-policy "$CLEANUP_POLICY" --replication "$REPLICATION")
