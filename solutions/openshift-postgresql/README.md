...
- An [Aiven account](https://console.aiven.io/signup). The free tier (`hobbyist` plan) is used throughout.
...
3. Select the **Hobbyist** (free) plan
...
The `hobbyist` plan (free tier) is a single-node instance with 8GB disk, no backups, and no high availability. It is available on all three providers.
